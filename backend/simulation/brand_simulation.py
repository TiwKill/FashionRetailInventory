...
from utils.helpers import get_season_info, get_festival_info, calculate_trend
...
class BrandSimulation:
    ...
    def _emit_month_trend_if_needed(self, current_date: datetime):
        """
        Emit monthly trend event at month-end.
        
        Called daily: if tomorrow is next month, calculates and emits
        trend data for the current month.
        """
        ...
        trend_label, trend_score, growth_vs_baseline, mom_growth = calculate_trend(
            sales=sales_m,
            baseline=baseline_m,
            prev_sales=self._prev_month_sales
        )
